...
namespace Sra.P2rmis.Web.Controllers
{
    public class SecurityController : SecurityBaseController
    {
        ...
        public JsonResult GetPolicyLookup()
        {
            var list = theLookupService.ListPolicy().ModelList;
            //var result = list.ConvertAll(x => new KeyValuePair<int, string>(x.Index, x.Value));
            return Json(list,JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetPolicyStatusLookup()
        {
            var list = theLookupService.ListPolicyStatus().ModelList;
            //var result = list.ConvertAll(x => new KeyValuePair<int, string>(x.Index, x.Value));
            return Json(list, JsonRequestBehavior.AllowGet);
        }

        #region Security Policy History 
        public ActionResult SecurityPolicyHistory()
        {
            var model = new SecurityPolicyHistoryViewModel();
            return View(model);
        }

        [Common.Authorize(Operations = Permissions.SecurityManagement.ViewSecurityInformation)]
        public ActionResult GetPolicyHistories()
        {
            var policies = theSecurityService.GetPolicyHistories()
                .OrderByDescending(p => p.StartDateTime)
                .Select(p => new PolicyGridViewModel()
                {
                    Id = p.PolicyId,
                    ...
                    StartDateTime = p.StartDateTime,
                    EndDateTime = p.EndDateTime,
                    RestrictionStartTime = p.RestrictionStartTime.ToString(),
                    RestrictionEndTime = p.RestrictionEndTime.ToString(),
                    DaysApplied = p.DaysApplied,
                    NetworkRanges = p.NetworkRanges,
                    Status = p.Status,
                    CreatedBy = p.CreatedBy,
        ...
        public ActionResult GetFilteredPolicyHistories(int? policyId, int? noOfDays, DateTime? fromDate, DateTime? toDate, PolicyStatus? policyStatus)
        {
            var policies = theSecurityService.GetFilteredPolicyHistories(policyId, noOfDays, fromDate, toDate, policyStatus);
            //if (policyId != null)
            //{
            //    policies = policies.Where(p => p.Id == policyId).ToList();
            //}
            //if (noOfDays != null)
            //{
            //    var Datetocompare = System.DateTime.UtcNow.AddDays(-noOfDays.Value);
            //    policies = policies.Where(p => p.StartDateTime.Date >= Datetocompare.Date).ToList();
            //}
            //if (fromDate != null && toDate == null)
            //{
            //    policies = policies.Where(p => p.StartDateTime.Date >= fromDate.Value.Date).ToList();
            //}
            //if (fromDate != null && toDate != null)
            //{
            //    policies = policies.Where(p => p.StartDateTime.Date >= fromDate.Value.Date && (p.EndDateTime != null || p.EndDateTime.Value.Date <= toDate.Value.Date)).ToList();
            //}
            //var opolicies = policies.Select(p => new PolicyGridViewModel()
            //{
            //    Id = p.Id,
            //    Name = p.Name,
            //    Type = p.Type,
