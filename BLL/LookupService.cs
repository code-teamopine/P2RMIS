...
namespace Sra.P2rmis.Bll
{
    ...
    public class LookupService : ServerBase, ILookupService
    {
        ...
        #region Services
        /// <summary>
        /// Retrieves vales for 'Policy' drop downs
        /// </summary>
        /// <returns>Container of IListEntry models</returns>
        public Container<IListEntry> ListPolicy()
        {
            Container<IListEntry> container = new Container<IListEntry>();
            var results = UnitOfWork.PolicyRepository.GetAll();
            container.ModelList = this.FillOrderedContainer<Policy>(results, x => x.PolicyId, x => x.Name, x => x.Name);

            return container;
        }
        /// <summary>
        /// Retrieves vales for 'Policy Status' drop downs
        /// </summary>
        /// <returns>Container of IListEntry models</returns>
        public Container<IListEntry> ListPolicyStatus()
        {
            Container<IListEntry> container = new Container<IListEntry>();
            var results = UnitOfWork.PolicyStatusRepository.GetAll();
            container.ModelList = this.FillOrderedContainer<PolicyStatu>(results, x => x.PolicyStatusId, x => x.PolicyStatusName, x => x.PolicyStatusId);
