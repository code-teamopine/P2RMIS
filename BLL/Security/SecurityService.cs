...
namespace Sra.P2rmis.Bll.Security
{
    public partial interface ISecurityService
    {
        ...
        IEnumerable<PolicyHistoryWebModel> GetPolicyHistories();
        IEnumerable<PolicyHistoryWebModel> GetFilteredPolicyHistories(int? policyId, int? noOfDays, DateTime? fromDate, DateTime? toDate, PolicyStatus? policyStatus);
    }
    public partial class SecurityService : ServerBase, ISecurityService
    {
        ...
        private PolicyHistory CreatePolicyHistory(Policy policy, int userId, PolicyStatus policyStatus, bool policyWeekDaysUpdated = false, bool policyNetworkRangeUpdated = false)
        {
            ...
            if ((policyStatus == PolicyStatus.Modified && policyWeekDaysUpdated) || policyStatus == PolicyStatus.Created)
            {
            ...
            if ((policyStatus == PolicyStatus.Modified && policyNetworkRangeUpdated) || policyStatus == PolicyStatus.Created)
            {
        ...
        /// <summary>
        /// Gets all history log policies
        /// </summary>
        /// <returns></returns>
        public IEnumerable<PolicyHistoryWebModel> GetPolicyHistories()
        {
            var entities = UnitOfWork.PolicyHistoryRepository.Get();
            var policies = entities.Select(p => new PolicyHistoryWebModel
            {
                PolicyId = p.PolicyId,
                Name = p.Name,
                StartDateTime = p.StartDateTime,
                ...
                RestrictionStartTime = p.RestrictionStartTime == null ? null : new DateTime().Add(p.RestrictionStartTime.Value).ToString("h:mm tt ET"),
                RestrictionEndTime = p.RestrictionEndTime == null ? null : new DateTime().Add(p.RestrictionEndTime.Value).ToString("h:mm tt ET"),
                DaysApplied = string.Join(", ", p.Policy.PolicyWeekDays
                    .Select(wd => wd.WeekDay.Abbreviation.Trim())
                    .ToList()),
                NetworkRanges = string.Join(" ", p.Policy.PolicyNetworkRanges
                    .Select(nr => $"{nr.StartAddress}, {nr.EndAddress}")
                    .ToList()),
                Status = p.PolicyStatu.PolicyStatusName,
                CreatedBy = UnitOfWork.UserRepository.GetByID(p.CreatedBy).FullName(),
                CreatedDateTime = p.CreatedDate
            });
            return policies;
        ...
        public IEnumerable<PolicyHistoryWebModel> GetFilteredPolicyHistories(int? policyId, int? noOfDays, DateTime? fromDate, DateTime? toDate, PolicyStatus? policyStatus)
        {
            var policyHistories = UnitOfWork.PolicyHistoryRepository.Select();
            if (policyId != null)
            {
                policyHistories = policyHistories.Where(p => p.PolicyHistoryId == policyId);
            }
            if (noOfDays != null)
            {
                var Datetocompare = System.DateTime.UtcNow.AddDays(-noOfDays.Value);
                policyHistories = policyHistories.Where(p => p.StartDateTime.Date >= Datetocompare.Date);
            }
            if (fromDate != null && toDate == null)
            {
                policyHistories = policyHistories.Where(p => p.StartDateTime.Date >= fromDate.Value.Date);
            }
            if (fromDate != null && toDate != null)
            {
                policyHistories = policyHistories.Where(p => p.StartDateTime.Date >= fromDate.Value.Date && (p.EndDateTime != null || p.EndDateTime.Value.Date <= toDate.Value.Date));
            }
            if(policyStatus!=null)
            {
                policyHistories = policyHistories.Where(p => p.PolicyStatusId==(int)policyStatus);
            }
            var ph = policyHistories
                .ToList();
            var list =  ph.Select(p => new PolicyHistoryWebModel()
            {
                Id=p.PolicyHistoryId,
                PolicyId = p.PolicyId,
                Name = p.Name,
                Type=p.PolicyType.Name,
                StartDateTime = p.StartDateTime,
                ...
                RestrictionStartTime = p.RestrictionStartTime == null ? null : new DateTime().Add(p.RestrictionStartTime.Value).ToString("h:mm tt ET"),
                RestrictionEndTime = p.RestrictionEndTime == null ? null : new DateTime().Add(p.RestrictionEndTime.Value).ToString("h:mm tt ET"),
                DaysApplied = string.Join(", ", p.PolicyWeekDayHistories
                    .Select(wd => wd.WeekDay.Abbreviation.Trim())
                    .ToList()),
                NetworkRanges = string.Join(" ", p.PolicyNetworkRangeHistories
                    .Select(nr => $"{nr.StartAddress}, {nr.EndAddress}")
                    .ToList()),
                Status = p.PolicyStatu.PolicyStatusName,
                CreatedBy = UnitOfWork.UserRepository.GetByID(p.CreatedBy).FullName(),
            ...
            var listPolicyHistoryIds = list.Where(w => string.IsNullOrEmpty(w.DaysApplied)).Select(s => s.Id);
            var policyWeekDayHistories = UnitOfWork.PolicyWeekDayHistoryRepository.Select()
                .OrderByDescending(o => o.CreatedDate)
                .Where(w => !listPolicyHistoryIds.Contains(w.PolicyHistoryId.Value)).ToList();
            foreach (var l in list.Where(w => string.IsNullOrEmpty(w.DaysApplied)))
            {
                var policyWeekDayHistory = policyWeekDayHistories.FirstOrDefault(f => f.PolicyId == l.PolicyId && f.PolicyHistoryId < l.Id);
                if (policyWeekDayHistory != null)
                {
                    l.DaysApplied = string.Join(", ", policyWeekDayHistories.Where(w => w.PolicyHistoryId == policyWeekDayHistory.PolicyHistoryId)
                    .Select(wd => wd.WeekDay.Abbreviation.Trim())
                    .ToList());
                }
            }

            listPolicyHistoryIds = list.Where(w => string.IsNullOrEmpty(w.NetworkRanges)).Select(s => s.Id);
            var policyNetworkRangeHistories = UnitOfWork.PolicyNetworkRangeHistoryRepository.Select()
                .OrderByDescending(o => o.CreatedDate)
                .Where(w => !listPolicyHistoryIds.Contains(w.PolicyHistoryId.Value)).ToList();
            foreach (var l in list.Where(w => string.IsNullOrEmpty(w.NetworkRanges)))
            {
                var policyNetworkRangeHistory = policyNetworkRangeHistories.FirstOrDefault(f => f.PolicyId == l.PolicyId && f.PolicyHistoryId < l.Id);
                if (policyNetworkRangeHistory != null)
                {
                    l.NetworkRanges = string.Join(", ", 
                        policyNetworkRangeHistories
                        .Where(w => w.PolicyHistoryId == policyNetworkRangeHistory.PolicyHistoryId)
                        .Select(wd => $"{wd.StartAddress}, {wd.EndAddress}")
                        .ToList());
                }
            }
